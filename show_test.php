...
if ($user_type === "employee") {
    ...
    if ($employee) {
        // Retrieve the tests that match the employee's business vertical, broad category, department, and sub-department and are enabled
        $stmt = $conn->prepare("SELECT test_id, Password, test_date FROM Test WHERE Business_vertical = :business_vertical AND Broad_category = :broad_category AND Department_name = :department_name AND Sub_department = :sub_department AND Enabled = 1 ORDER BY test_date DESC");
                    ...
                    <div class="modal-body">
                        ...
                        <?php if ($questions && count($questions) > 0): ?>
                            <form method="post" action="submit_test.php">
                                ...
                                <button type="submit" class="btn btn-primary">Submit Test</button><span class="text-danger"> Important:- Once you submit then you won't be able to take this test again</span>
                            </form>
                        <?php else: ?>
